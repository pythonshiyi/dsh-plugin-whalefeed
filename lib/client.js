  ...
  factory: (require) => {
    ...
    if (
      ...
      tag.textContent = `
        ...
        .dsh-wf-info, .dsh-wf-gallery {
          ...
          border: 1px solid var(--dsh-wf-panel-border);
          border-radius: 14px;
          box-shadow: 0 10px 32px var(--dsh-wf-shadow);
          padding: 12px 14px;
          font-size: 12px;
          line-height: 1.6;
          z-index: 10;
        ...
        .dsh-wf-info dl { margin: 0; display: grid; grid-template-columns: auto 1fr; gap: 2px 10px; }
        .dsh-wf-info dt { opacity: 0.7; }
        .dsh-wf-info dd { margin: 0; text-align: right; font-variant-numeric: tabular-nums; }
    ...
    function WhaleFeed(props) {
      ...
      useEffect(() => {
        ...
        persistState(next);
        lastTotalRef.current = totalTokens;
        setMood(stageUp ? "excited" : "eating");
        setBubble(
          stageUp
            ? t("stageUp").replace("{label}", stageLabel(next.stage, options.stages, t))
      ...
      const handleReset = useCallback(() => {
        if (typeof window !== "undefined" && typeof window.confirm === "function" && !window.confirm(t("resetConfirm")))
          return;
        ...
        setMood("idle");
      }, [sessionId, totalTokens, t, persistState]);
      ...
      const handleImport = useCallback(() => {
        const raw =
          typeof window !== "undefined" && typeof window.prompt === "function"
            ? window.prompt(t("importPrompt"), "")
            : null;
        if (!raw) return;
        const parsed = parseStoredState(raw, sessionId);
        if (!parsed.exists) {
          if (typeof window !== "undefined" && typeof window.alert === "function") window.alert(t("importFailed"));
          return;
        }
      ...
      const infoPanel = showInfo
        ? React.createElement(
            ...
            React.createElement(
              "div",
              { className: "dsh-wf-actions" },
              React.createElement("button", { className: "dsh-wf-btn", onClick: handleExport }, t("export")),
              React.createElement("button", { className: "dsh-wf-btn", onClick: handleImport }, t("import")),
              React.createElement("button", { className: "dsh-wf-btn", onClick: toggleGallery }, t("gallery")),
            ),
            React.createElement("button", { className: "dsh-wf-reset", onClick: handleReset }, t("reset")),
          )
        : null;
    ...
    function apply(ctx, config) {
      ...
      try {
        slots.inject("shell.overlay", () => makeRegistration("shell.overlay", "whale-feed", "floating"));
      ...
      if (!overlayInjected) {
        try {
          slots.inject("conversation.session.header.utilities", () =>
            makeRegistration("conversation.session.header.utilities", "whale-feed-badge", "badge"),
          );
        } catch {
          /* no compatible slot */
        }
